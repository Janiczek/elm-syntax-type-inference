...
suite =
    Test.describe "Elm.TypeInference.SCC"
        ...
        , Test.fuzz graphFuzzer "every node appears exactly once" <| \graph ->
        let
            sccs : List (List Int)
            sccs =
            ...
            allReturned =
                List.ExtraExtra.fastConcat sccs
        in
        allReturned
            |> List.sort
            |> Expect.equal (List.sort (nodesOf graph))
        , Test.fuzz graphFuzzer "every cross-group edge points backwards (dependencies first)" <| \graph ->
        let
            sccs : List (List Int)
            sccs =
                SCC.stronglyConnectedComponents (nodesOf graph) (edgesOf graph)
        ...
        violations
            |> Expect.equalLists []
        , Test.fuzz graphFuzzer "every group is strongly connected" <| \graph ->
        let
            sccs : List (List Int)
            sccs =
                SCC.stronglyConnectedComponents (nodesOf graph) (edgesOf graph)
        ...
        badGroups
            |> Expect.equalLists []
        , Test.fuzz graphFuzzer "every two mutually-reachable nodes end up in the same group (maximality)" <| \graph ->
        let
            sccs : List (List Int)
            sccs =
                SCC.stronglyConnectedComponents (nodesOf graph) (edgesOf graph)
